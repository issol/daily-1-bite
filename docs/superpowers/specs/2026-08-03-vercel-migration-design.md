...
### 2-5-1. 실행 기록 (2026-08-10 완료)

1단계 배포(8/3) 후 **7일** 경과 시점에 배포했다. 3~7일 창의 마지막 날을 택한 이유는
GSC 데이터가 2~3일 지연되기 때문이다 — 8/6에 했으면 호스팅 변경이 크롤에 영향을
줬는지 판단할 데이터가 없는 상태에서 URL 변경을 얹는 셈이 된다.

**배포 전 GSC 판정 (크롤링 통계, 8/10 내보내기)**

- 호스트 상태: `daily1bite.com`(1066 요청) / `www`(38 요청) 모두 **"문제 없음"**
- 크롤 평균 응답: 컷오버 전 30일 **274ms** → 후 5일 **72ms** (중앙값 172 → 65)
- 일평균 크롤 요청: 11.4 → 10.2 (변동 없음)
- 즉 호스팅 이전은 크롤 사고를 내지 않았고 오히려 4배 빨라졌다

**응답 코드 분포에서 발견한 것 (90일 누적)**

`302 4.17%`가 I4("리디렉션은 전부 영구")와 어긋났다. 원인을 특정했다:
**next-intl 미들웨어가 `oldRoutes`에 없는 모든 경로를 307로 `/ko/<경로>`에 보내고
있었다.** `/rss` → 307 → `/ko/rss` → 404 같은 체인이 만들어져 302와 404를 동시에
생산하고 있었다. Amplify 시절부터 있던 구조적 문제다.

2단계로 이게 사라졌다. 실측:

| 경로 | 1단계 | 2단계 |
|---|---|---|
| `/KO`, `/Ko/blog` | 307 → `/ko...` | **301** → `/...` |
| `/rss` | 307 → `/ko/rss` → 404 | **404 직접** |
| `/category` | 301 → `/ko/category` → 404 | **404 직접** |

Next.js `redirects()`는 대소문자를 무시하므로 `/KO` 계열도 301로 잡힌다.
`/category`의 301 → 404 체인(기존 결함)도 부수적으로 해소됐다.

**배포 후 검증 (프로덕션 실측)**

- `verify-urls.sh`: 매핑 393건 전부 일치 / 리디렉션 목적지 152건 전부 즉시 200 /
  404 0건 / sitemap 108·`/ko` 0·`/en` 0·hreflang 0 / 헤더 6종
- canonical이 루트 URL(`https://daily1bite.com/blog/...`)
- GA4(`/stats`) 정상, `www` 200 유지(리디렉트 없음)
- 임시 리디렉션(302/307) **0건**
