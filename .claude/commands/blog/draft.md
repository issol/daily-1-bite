...
| 💬 커뮤니티 반응 | Reddit/HN/X에서 **실제로 나온 말**을 원문 인용 + 참여수 + 링크 | "r/LocalLLaMA 412업보트 댓글: \"...\"" |

> **💬 레이어 인정 조건 (엄격)**: `last30days`로 실제 수집한 **원문 인용 2개 이상** +
> 각각 작성자 핸들 + **실제 URL** + 참여수가 본문에 있을 때만 1개 레이어로 인정한다.
> "커뮤니티에서 반응이 좋다" 같은 요약 서술은 레이어가 아니다. 수집에 실패했으면 인정 안 됨 —
> 지어내면 그게 바로 이 블로그가 강등된 이유의 재현이다. 수집 절차는 Step 2.75.
...
### Step 2.75: 커뮤니티 반응 리서치 (💬 레이어) — `last30days` 스킬

> **왜**: Step 2.7이 모으는 건 전부 **발표자가 하고 싶은 말**이다(공식 블로그, 릴리스 노트,
> 보도자료를 받아쓴 기사). 그것만으로 쓴 글이 "원본 발표의 재요약"으로 판정돼 이 블로그의
> 색인이 붕괴했다. 이 단계는 **쓴 사람들이 실제로 한 말**을 가져온다. 공식 발표에는 절대
> 없는 정보이고, AI가 보도자료를 요약해서는 만들어낼 수 없는 유일한 종류의 재료다.

**2.75-A. 이미 수집된 재료가 있는지 먼저 확인**

번호 입력으로 시작한 경우, `~/blog-drafts/.last-topics.json`의 해당 주제에
`communityEvidence`가 있으면 그것을 사용한다(topic 단계에서 이미 수집됨). 그 경우 2.75-B 생략.
`"communitySignal": "none"`이면 💬 레이어를 기대하지 말고 2.75-D로.

**2.75-B. 수집 (재료가 없을 때)**

Skill 도구로 `last30days` 호출, args: `{글의 핵심 고유명사}`
- 예: `Claude Opus 4.8 fast mode`, `Cursor 3.5 shared canvases`, `MCP linux foundation`
- 제품명·모델명·기능명 같은 **고유명사**로 물어야 한다. "AI 코딩 도구 동향" 같은 일반명사는
  잡음만 돌아온다.
- 수 분 걸린다. 정상이다. 중간에 끊고 웹검색으로 대체하지 말 것.

**2.75-C. 본문 반영 형식 (엄수)**

인용은 **원문 + 한국어 옮김**을 나란히 놓는다. 원문 없이 한국어만 쓰면 그건 인용이 아니라
내가 지어낸 문장이 된다.

```markdown
> "Everyone's acting like this is free. It's not - you're paying in latency."
> — [u/example](https://reddit.com/r/LocalLLaMA/comments/.../) · r/LocalLLaMA 412 업보트

다들 공짜인 것처럼 말하는데 아니라는 것 — 지연시간으로 지불하고 있다는 지적입니다.
```

**규칙:**
- `text`는 **원문 그대로**. 다듬거나 줄이지 않는다. 줄여야 하면 `…`로 생략 표시.
- 작성자 핸들(`u/name`, `@handle`)과 **URL을 반드시** 붙인다. URL은 수집 결과에서
  **복사**만 한다. **절대 조합하거나 추측하지 않는다** — 그럴듯한 가짜 링크가 제일 나쁘다.
- 참여수(업보트/댓글수/조회수)를 함께 적는다. 그게 "이게 소수 의견이 아니다"의 근거다.
- **찬반이 갈린 지점을 우선 인용하라.** 한쪽 의견만 모으면 그냥 응원글이 된다.
  글의 가장 좋은 소재는 커뮤니티가 싸우고 있는 논점이다.
- `last30days`의 **출력 형식(뱃지, `What I learned:`, 이모지 트리 푸터)을 MDX에 옮기지 말 것.**
  그건 그 스킬의 리포트 서식이다. 우리는 그 안의 **증거만** 가져와 우리 글의 문체로 쓴다.

**2.75-D. 수집 실패 시 (중요)**

스킬이 없거나, 첫 실행 설정 미완료거나, 결과가 비었으면:
- **글 작성을 중단하지 않는다.** 💬 레이어만 포기한다.
- 나머지 레이어(🧪⚖️🧭🔪🛠)로 Step 0의 2개 요건을 채운다. 못 채우면 `noindex: true`.
- 본문에 "커뮤니티 반응은 확인하지 못했습니다" 같은 도구 사정을 쓰지 않는다.
  독자에게는 우리 파이프라인 사정이 아무 의미 없다. 조용히 빼면 된다.

### Step 2.8: 실행 주입 (🧪 레이어) — Claude Code가 직접 실행한다 (API 키 불필요)

> **핵심**: 이 스킬은 **Claude Code 안에서 실행 중**이고, Claude Code는 실제 실행 능력
> (Bash로 코드·CLI·명령 실행, 파일 읽기, WebFetch)을 가진 Claude 모델 그 자체다.
> 즉 "자동화가 만들 수 없던 1인칭 Experience"를 **별도 API 키 없이** 지금 만들 수 있다.
> 절대 규칙: **실제로 실행한 것만 결과로 쓴다. 안 돌려본 수치·출력은 날조 금지.**

**글 주제에서 실행 가능한 것을 찾아 지금 실행하고, 검증된 실결과를 본문에 넣는다:**

- **코드 튜토리얼/스니펫** → Bash로 실제 실행(`node`, `python3`, `bash`)하고 **진짜 출력/에러**를
  캡처해 삽입. 명령과 결과를 함께 보여준다.
  ```bash
  # 예: 글에 넣을 스니펫을 실제로 돌려 결과를 확인
  node -e 'console.log([3,1,2].sort((a,b)=>a-b))'   # → 실제 stdout을 본문에 인용
  ```
- **CLI/도구 사용법**(설치돼 있으면) → 실제 명령을 실행해 버전·출력·동작을 캡처.
  없으면 "이 글에선 실행 불가"로 명시하고 다른 레이어로 보강.
- **Claude/LLM 출력 데모** → 지금 실행 중인 모델(Claude)이 대표 프롬프트에 **실제로 답한 출력**을
  생성 시점에 만들어 인용한다("작성 중 Claude로 생성한 실제 출력"이라 명시).
- **명령/설정 검증** → 실제로 실행해 되는지 확인한 뒤 "동작 확인함"을 쓴다.

**실행 불가한 주장**(예: 타사 모델의 지연시간·벤치 — 로컬에서 못 돌림)은 🧪로 쓰지 말고,
공식 발표 기반 ⚖️(스펙 비교)·🧭(분석)·🔪(비판)으로 채운다. **측정한 것과 발표 기반을 명확히 구분**한다.

> 정직성 체크: 본문의 모든 "실행 결과/수치"는 이번 세션에서 실제로 돌린 명령의 출력이어야 한다.
> Step 0 게이트의 🧪 레이어는 이 "실제 실행"으로만 충족된다.
>
> ⚠️ **본문에 "꾸며낸 값 아님/실제로 돌렸다" 같은 방어적 문구를 쓰지 말 것.** 결과를 그냥
> 사실로 제시한다(예: "실행하면 이렇게 나옵니다:"). 독자가 의심하지도 않은 걸 방어하면
> 오히려 신뢰가 깎이고 AI 냄새가 난다. 정직성은 실제 실행으로 지키는 것이지 문구로 지키는 게 아니다.
>
> (참고: `scripts/measure.mjs`는 `ANTHROPIC_API_KEY`가 있는 환경에서 다중 모델 지연/비용 벤치를
> 뽑는 **선택적** 헬퍼일 뿐이다. 기본 경로는 위의 Claude Code 직접 실행이며 키가 필요 없다.)
...
0-B. **💬 인용 무결성 (💬를 레이어로 카운트한 경우만)**: 인용 블록마다
   ① 원문(영문 그대로) ② 작성자 핸들 ③ URL ④ 참여수가 **전부** 있는지 확인.
   하나라도 빠지면 그 인용은 레이어로 카운트하지 않는다. 인용이 2개 미만이면 💬 불인정 →
   0번 게이트 재판정. **URL은 실제 접근 가능해야 한다** — 조합·추측한 링크가 발견되면
   해당 인용 삭제 후 FAIL 처리. 한국어 번역만 있고 원문이 없는 인용도 FAIL.
