...
> **순서 주의 (2026-07-23 첫 실전에서 교정)**: 심층 조회(2-D-1)가 **주력**이고 트렌딩
> 발굴(2-D-3)이 보조다. 처음엔 반대로 적어뒀는데, 실제로 돌려보니 트렌딩 발굴은 후보 1건에
> 그마저 AI와 무관한 항목을 내놓은 반면, 고유명사 심층 조회는 Reddit 12스레드·322업보트·
> YouTube 9편을 물어와 그 글의 논지를 통째로 바꿨다. 시간도 심층 조회가 훨씬 싸다.

**2-D-1. 소재 심층 조회 (주력 — 여기서 대부분의 재료가 나온다)**

2-A~2-C에서 나온 **고유명사**를 골라 그 이름으로 조회한다.
Skill 도구로 `last30days` 호출, args: `{제품명/모델명/기능명}`
- 예: `Claude Opus 4.8 fast mode`, `Cursor 3.5 shared canvases`, `hybrid search BM25 RRF reranking RAG`
- **고유명사여야 한다.** "AI 코딩 도구 동향" 같은 일반명사는 잡음만 돌아온다.
- 유력 후보가 2~3개면 각각 조회한다. 한 번에 수 분이니 병렬로 돌리지 말고 순차로.

**2-D-2. 파이프라인 큐 확인 (중복 방지)**
...
(큐는 트렌딩 발굴을 **끝까지 완주**해야 채워진다. 2-D-3을 중간에 접으면 비어 있는 게 정상이다.)

**2-D-3. 트렌딩 발굴 (보조 — 새 소재가 마를 때만)**

2-A~2-D-1로 쓸 만한 후보가 3개 이상 나왔으면 **건너뛴다.** 소재가 마르거나 새 영역을 뚫고
싶을 때만 돌린다.

Skill 도구로 `last30days` 호출, args: `what's exploding in AI developer tools`
(키워드 힌트가 있으면 그 도메인으로 바꾼다)

3단계 프로토콜(nominate → judge → finalize)로 돌며 수 분이 걸린다. 결과가 얇게(후보 0~1건)
나오는 건 흔하다 — river feed 소스가 몇 개나 살아있느냐에 좌우된다. **얇게 나오면 그건 정직한
결과다.** 억지로 재시도하거나 후보를 지어내지 말고 2-E로 넘어간다.
...
**소스 상태 확인 (처음 한 번, 또는 결과가 이상할 때):**

Skill 도구로 `last30days` 호출, args: `doctor`
→ 어떤 소스가 살아있는지 표로 나온다. 켜져 있어야 정상인 것: reddit(+댓글), hackernews,
youtube, github, digg, techmeme, arxiv. X는 브라우저 쿠키를 안 쓰기로 했으면 꺼진 게 정상이다.
꺼진 소스가 있으면 doctor가 찍어준 fix 명령을 그대로 실행한다.
